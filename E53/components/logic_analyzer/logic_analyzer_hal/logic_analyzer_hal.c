...
{
    int noTimeout;
    // return 0 if timeout, otherwise return a non-zero value
    ...
    while (1)
    {
        ...
        if (noTimeout)                                                // dma data ready
        {
...
#endif
            int l_samples = (cfg->number_channels > 4) ? (la_frame.fb.len / (cfg->number_channels / 8)) : la_frame.fb.len * 2; // 计算采样数量
            // l_samples 是采样数量的计算结果，根据通道数量决定计算方式
            // 如果通道数量大于4，那么采样数量等于帧缓冲区长度除以通道数量除以8（即每个通道的采样数量）
            // 如果通道数量小于等于4，那么采样数量等于帧缓冲区长度乘以2（即每个采样点包含两个通道的数据）
            ...
            // 调用回调函数，传入参数包括：
            // 帧缓冲区的指针，转换为uint8_t类型
            // 采样数量
            // 通过调用logic_analyzer_ll_get_sample_rate函数获取的采样率
            // 通道数量
            logic_analyzer_stop();
...
esp_err_t start_logic_analyzer(logic_analyzer_config_t *config)
{
    ...
    // 如果通道数量大于4，那么每个通道的采样数据量是采样数量乘以通道数量除以8。这是因为每个通道的采样数据量是以8位（1字节）为单位的，所以通道数量除以8得到的是每个通道的采样数据量。
    // 如果通道数量小于等于4，那么采样数量是采样数量除以2。这是因为每个采样点包含两个通道的数据。

    if (config->samples_to_psram == 0)
    {
        // 如果不使用PSRAM，则在RAM中分配内存
        uint32_t largest_free_block = heap_caps_get_largest_free_block(MALLOC_CAP_DMA); // 获取最大的可用内存块大小（字节）
        // 如果最大可用内存块小于需要分配的字节数加上DMA描述符的大小，则调整需要分配的字节数
        if (largest_free_block < bytes_to_alloc + ((bytes_to_alloc / DMA_FRAME) + 1) * sizeof(lldesc_t))
        {
            bytes_to_alloc = largest_free_block - ((bytes_to_alloc / DMA_FRAME) + 2) * sizeof(lldesc_t); // 调整后的字节数
        }
        // 打印分配前的DMA堆内存信息
        ...
        // 计算分配的字节数，确保是4字节对齐的
        la_frame.fb.len = bytes_to_alloc & ~0x3; // 使用位运算确保字节数是4字节对齐的
        // 分配内存，并初始化为0
